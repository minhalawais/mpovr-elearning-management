...
const FAQItem: React.FC<{ item: FAQItem; isOpen: boolean; toggleOpen: () => void }> = ({ item, isOpen, toggleOpen }) => (
  <div className="border-b border-[#399fc6]/20 last:border-b-0">
    <button
      ...
      onClick={toggleOpen}
    >
      <span className="font-semibold text-[#3756C0] pr-6 text-base">{item.question}</span>
      {isOpen ? (
        <ChevronUp className="w-5 h-5 text-[#E18400] flex-shrink-0 transition-transform duration-300" />
      ) : (
        <ChevronDown className="w-5 h-5 text-[#E18400] flex-shrink-0 transition-transform duration-300" />
      )}
    </button>
    {isOpen && (
...
const FAQSection: React.FC = () => {
  ...
  return (
    <section className="py-12 bg-gradient-to-b from-[#edf6f9] to-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-8">
          <div className="inline-block p-2 rounded-full bg-[#399fc6]/10 mb-2">
            <HelpCircle className="w-8 h-8 text-[#399fc6]" />
          </div>
          <h2 className="text-3xl font-bold text-[#3756C0] mb-4">Frequently Asked Questions</h2>
        ...
        <div className="max-w-3xl mx-auto mb-8">
          <div className="relative">
            <input
              ...
              className="w-full py-3 px-4 pr-10 rounded-full border border-[#399fc6] focus:outline-none focus:ring-2 focus:ring-[#399fc6] focus:border-transparent text-sm transition-all duration-300"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <Search className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#399fc6] w-5 h-5" />
          </div>
        </div>
