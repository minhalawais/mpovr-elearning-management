...
const VirtualSessionPage: React.FC = () => {
  ...
  const handleSessionClick = (session: VirtualSession) => {
    ...
    fetchTraineeAttendance(session.session_id);
    setIsEditMode(false);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://127.0.0.1:8000/virtual_session/${editedSession.session_id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      if (response.ok) {
        ...
        setSessions(sessions.map(s => s.session_id === updatedSession.session_id ? updatedSession : s));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {filteredSessions.map((session) => (
              <motion.div
                  ...
                  ${selectedSession?.session_id === session.session_id ? 'border-2 border-[#8b5cf6]' : 'border-2 border-transparent'}
                  `}
